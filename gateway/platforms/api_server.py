...
class APIServerAdapter(BasePlatformAdapter):
    ...
    async def _write_sse_chat_completion(
        ...
        try:
            ...
            # Finish chunk — suppressed when a tool_calls finish was already
            # emitted (MECG AG-UI/client-tools passthrough path) so the client
            # doesn't see a spurious trailing finish. Otherwise emit upstream's
            # richer finish chunk (error/length finish_reason + hermes metadata).
