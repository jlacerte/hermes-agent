...
SEND_MESSAGE_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "target": {
                "type": "string",
                "description": "Delivery target. Format: 'platform' (uses home channel), 'platform:#channel-name', 'platform:chat_id', or 'platform:chat_id:thread_id' for Telegram topics and Discord threads. Examples: 'telegram', 'telegram:-1001234567890:17585', 'discord:999888777:555444333', 'discord:#bot-home', 'slack:#engineering', 'signal:+155****4567', 'matrix:!roomid:server.org', 'matrix:@user:server.org', 'ntfy:alerts-channel' (explicit ntfy topic), 'yuanbao:direct:<account_id>' (DM), 'yuanbao:group:<group_code>' (group chat), 'email:justin@mecg.ca' (send an EMAIL to that address — use this whenever asked to send/reply by courriel/email; combine with 'subject')"
            },
            "message": {
                "type": "string",
...
async def _send_to_platform(platform, pconfig, chat_id, message, thread_id=None, media_files=None, force_document=False, subject=None):
    ...
    for chunk in chunks:
        if platform == Platform.SLACK:
            ...
            # MECG: explicit `subject` param (was carried by the removed local
            # _send_email). Upstream moved email sending to the email adapter,
            # which derives the subject from a leading "Subject:" line in the
            # body (EmailAdapter._parse_subject_and_body). Bridge our param to
            # that convention so the subject feature survives the refactor.
            email_chunk = chunk
            if subject and not chunk.lstrip().lower().startswith("subject:"):
                email_chunk = f"Subject: {subject}\n{chunk}"
            result = await _registry_standalone_send("email", pconfig, chat_id, email_chunk, thread_id)
...
# MECG note: the explicit `subject` param (former _send_email(subject=...)) is
# now bridged to the adapter's "Subject:" body-prefix convention in
# _send_to_platform (Platform.EMAIL branch).
